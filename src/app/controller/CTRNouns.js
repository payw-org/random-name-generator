...
async function selectAll(req, res) {
    try {
        var [result] = await DBConnection.query(`SELECT * FROM nouns`)
        return result
    } catch (e) {
        console.log(`>Error - ${e}`)
    }
...
async function insertByName(req, res) {
    try {
        ...
        var [result] = await DBConnection.query(`SELECT _id FROM nouns WHERE name = '${name}'`)

        if (result.length === 0) {
            await DBConnection.query(`INSERT INTO nouns (name) VALUES (${name})`)
            res.sendStatus(200)
        }
...
async function deleteByName(req, res) {
    try {
        ...
        if (result.length !== 0) {
            await DBConnection.query(`DELETE FROM nouns WHERE _id = ${result[0]._id}`)
            res.sendStatus(200)
        }
        else { // not exist
